...
`authorization.*` 领域会投影提供方拥有的登录 flow，且绝不暴露凭据 payload。`list` 将每个 flow 与不含值的记录状态联接；`start` 为每个凭据键创建一次尝试，`status` 读取其 notice 与当前 prompt，`respond` 回答该确切 prompt，`cancel` 经 `ctx.authorization` 撤回，`signOut` 则删除本地记录。终态尝试按每个已安装 flow 键至多保留一个值，并由下一次 start 替换。浏览器载体把所有这些方法限制在回环地址，因为 list 会暴露已配置账号，start/respond 能产生 grant，而 sign-out 会删除 grant。
