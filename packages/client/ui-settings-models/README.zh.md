...
模型设置与产品引导插件。同一个 client Cordis 插件会注册 Models 页面和两个有序的首次使用弹窗：版本化内测声明，以及按条件显示的 DeepSeek 官方凭据步骤。两个步骤共用同一套弹窗组件，并继续由 `settings.onboarding` 排序。Models 平面把四个协议领域汇聚为一个共享快照：`llm.providers`（可配置提供方目录，含每条路由的存活／休眠状态）、`settings.describe`（序列化 schema、分层脱敏值、secret slot）、`credentials.describe`（不含值的 configured/source/writable 徽标）与 `authorization.list`（提供方原生登录方法及 grant 状态）；页面据此渲染提供方行，一次只展开一张编辑卡片，且不把路由存活状态呈现为提供方状态。

带授权 flow 的 pi-ai 路由会在 API 密钥字段旁显示提供方拥有的登录方法；控件会渲染登录 URL、设备码、文本／机密／选择提示、进度、取消、失败与本地退出登录。grant 完成后，卡片会先具化不带引用的 profile 再关闭，因此 `openai-codex` 这类仅 OAuth 的休眠路由无需 API 密钥即可激活。
