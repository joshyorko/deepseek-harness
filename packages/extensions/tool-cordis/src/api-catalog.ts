...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    summary: 'Optional self-hosted Remote owner for browser-driven authorization flows.',
    description: 'Optional self-hosted Remote owner for browser-driven authorization flows.',
    methods: [
      {
        signature: '@Remote(\'list\') async list(): Promise<readonly AuthorizationFlowView[]>',
        description: 'List registered provider authorization flows and current record state.',
        parameters: [],
        returns: 'the current flow views.',
      },
      {
        signature: '@Remote(\'start\') start(key: string, method: string): AuthorizationStartView',
        description: 'Start one provider authorization attempt.',
        parameters: [{ name: 'key', description: 'credential key of the registered flow.' }, { name: 'method', description: 'provider authorization method to run.' }],
      ...
      {
        signature: '@Remote(\'status\') status(key: string, attemptId: string): AuthorizationAttemptView',
        description: 'Read the current state of one authorization attempt.',
        parameters: [{ name: 'key', description: 'credential key of the registered flow.' }, { name: 'attemptId', description: 'attempt identifier returned by {@link start}.' }],
        returns: 'the current attempt state.',
        throws: ['{RemoteError} when the key or attempt is stale.'],
      },
      {
        signature: '@Remote(\'respond\') respond(key: string, attemptId: string, promptId: string, value: string): AuthorizationAttemptView',
        description: 'Submit a response to the active authorization prompt.',
        parameters: [{ name: 'key', description: 'credential key of the registered flow.' }, { name: 'attemptId', description: 'active attempt identifier.' }, { name: 'promptId', description: 'active prompt identifier.' }, { name: 'value', description: 'selected or entered response.' }],
        returns: 'the updated attempt state.',
        throws: ['{RemoteError} when the attempt or response is invalid.'],
      },
      {
        signature: '@Remote(\'cancel\') cancel(key: string, attemptId: string): AuthorizationAttemptView',
        description: 'Cancel one active authorization attempt.',
        parameters: [{ name: 'key', description: 'credential key of the registered flow.' }, { name: 'attemptId', description: 'active attempt identifier.' }],
        returns: 'the cancelled attempt state.',
        throws: ['{RemoteError} when the key or attempt is stale.'],
      },
      {
        signature: '@Remote(\'signOut\') async signOut(key: string): Promise<void>',
        description: 'Delete the stored credential for a provider flow.',
        parameters: [{ name: 'key', description: 'credential key of the registered flow.' }],
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  },
  {
    ...
    declaration: 'export type AuthorizationAttemptState = \'running\' | \'prompt\' | \'authorized\' | \'cancelled\' | \'failed\';',
  },
  {
  ...
  {
    name: 'AuthorizationStartView',
    declaration: 'export interface AuthorizationStartView {\n    readonly attemptId: string;\n    readonly key: string;\n    readonly status: AuthorizationAttemptView;\n}',
  },
  {
