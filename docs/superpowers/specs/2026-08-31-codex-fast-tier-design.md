...
The pi-ai adapter continues using `Models.streamSimple()` for ordinary routes. When a validated `openai-responses` or `openai-codex-responses` profile selects a service tier, the adapter uses pi-ai's API-specific `Models.stream()` entry and supplies the tier as `serviceTier`. This preserves pi-ai's request construction, response-tier resolution, and usage accounting instead of modifying the completed JSON through `onPayload`.
