...
flowchart LR
  ...
  svc_authorizationController["ctx.authorizationController<br/>Host provider authorization controller"]
  ...
  pkg_llm_pi_ai_oauth --> svc_authorizationController
...
| `ctx.authorizationController` | `core` | [`llm-pi-ai-oauth`](../packages/llm/llm-pi-ai-oauth) | - | - | - | Owns ephemeral browser authorization attempts and redacted provider-flow state while credential grants remain with the authorization and credential seams. |
