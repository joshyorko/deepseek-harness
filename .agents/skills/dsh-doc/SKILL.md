...
6. Update a Chinese counterpart only when the user explicitly requests translation. English documentation otherwise changes independently.
...
5. **Record explicitly requested translations.** When the user requests a paired update, run `pnpm run verify-translation-pairing --write <pair>` after confirming both sides.
...
- Explicitly requested bilingual updates: verify structure, terminology, link parity, and the sidecar record.
