...
Docs accompany every code change: update affected English README and JSDoc contracts together. Chinese counterparts are optional and change only when explicitly requested; `dsh-translate-docs` also remains user-invoked. Current-state prose, one physical line per paragraph, one home per fact, and word budgets live in [docs/AGENTS.md](docs/AGENTS.md).
