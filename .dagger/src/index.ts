...
export class Dsh {
  ...
  private releaseBundle(source: Directory, commitHash: string): Container {
    ...
    return dag
      ...
      // The private workspace root is not published, but tsdown still resolves
      // its fallback entries before building the workspace packages.
      .withExec([
        "sh",
        "-c",
        "mkdir -p lib/types && for f in index invariant startup; do printf 'export {}\\n' > \"lib/types/$f.js\"; done",
      ])
